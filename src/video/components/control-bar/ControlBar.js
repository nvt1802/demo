...
export default class ControlBar extends Component {
  ...
  getDefaultChildren() {
    return [
      // <VolumeMenuButton key="volume-menu-button" order={1} />,
      <CurrentTimeDisplay key="current-time-display" order={2} />,
      <PlayToggle key="play-toggle" order={4} />,

      // <TimeDivider key="time-divider" order={5.2} />,
      <DurationDisplay key="duration-display" order={6} />,
  ...
  getChildren() {
    ...
    return (
      <div style={{ width: "100%" }}>
        <div style={{ width: "100%" }}>
          <ProgressControl key="progress-control" {...parentProps} />
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          <div>
            <VolumeMenuButton {...parentProps} vertical />
          </div>
          <div style={{ display: "flex", justifyContent: "center" }}>
            {mergeAndSortChildren(defaultChildren, children, parentProps)}
          </div>
          <div>
            <DownloadButton {...parentProps} />
            <FullscreenToggle {...parentProps} />
          </div>
        </div>
      </div>
    )
